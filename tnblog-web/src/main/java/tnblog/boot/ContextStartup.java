...
import tnblog.base.print.Printer;
import tnblog.core.data.Config;
import tnblog.core.persist.service.ChannelService;
import tnblog.core.persist.service.ConfigService;
    ...
    public void run(ApplicationArguments applicationArguments) throws Exception {
        ...
        timer.schedule(new TimerTask() {
            ...
            public void run() {
                ...
                if (configs.isEmpty()) {
                    Printer.error("------------------------------------------------------------");
                    Printer.error("- ERROR: No initialization data is imported (db_tnblog.sql) -");
                    ...
                    if (configs.size() < 13) {
                        Printer.error("-----------------------------------------------------------------");
                        Printer.error("- ERROR: The data is not complete, Please import (db_tnblog.sql) -");
                        Printer.error("-----------------------------------------------------------------");
                    }
